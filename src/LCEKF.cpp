...
void LCEKF::Configure() {
  ...
  Rw_.block(9,9,3,3) = 2.0f * (wMarkovSigma_rps2 * wMarkovSigma_rps2) / wMarkovTau_s * I3;
...
void LCEKF::Initialization(Vector3d Meas_omega_ib_b, Vector3d Meas_f_ib_b, Vector3d Meas_r_eb_e, Vector3d Meas_v_eb_e, Vector3d Bestpos_ECEF_SD, Vector3d Bestvel_ECEF_SD, double dualheading) {
...
void LCEKF::Update(int num, Vector3d Meas_omega_ib_b, Vector3d Meas_f_ib_b, Vector3d Meas_r_eb_e, Vector3d Meas_v_eb_e, Vector3d Bestpos_ECEF_SD, Vector3d Bestvel_ECEF_SD, double dualheading) {
...
void LCEKF::System_Update(double dualheading) {
	/*
	  ...
	  if (dualheading != 0)
		  ...
		  euler_BL_rad_(2) += Est_omega_ib_b_(2) * dt_s_;
	  */
	  ...
	  Matrix3d Est_C_ECEF2NED_ = TransE2NED(Est_r_eb_e_);
	  ...
	  Matrix<double,3,3> F; F.block(0,0,3,3) = Est_ECEF * Est_ECEF.transpose();
	  ...
	  Fs.block(3,0,3,3) = -2 * Est_C_NED2ECEF * Est_C_B2NED * Skew(Est_f_ib_b_);
	  Fs.block(3,9,3,3) = -Est_C_NED2ECEF * Est_C_B2NED;
	  Fs.block(0,0,3,3) = -Skew(Omega_ie);
...
// Measurement Update
void LCEKF::Measurement_Update(Vector3d Meas_r_eb_e, Vector3d Meas_v_eb_e) {
