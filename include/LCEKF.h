...
class LCEKF {
  public:
	//LCEKF() {};
    void Configure(Vector3d Bestpos_ECEF_SD, Vector3d Bestvel_ECEF_SD);
    void Initialization(Vector3d Meas_omega_ib_b, Vector3d Meas_f_ib_b, Vector3d Meas_r_eb_e, Vector3d Meas_v_eb_e, Vector3d Bestpos_ECEF_SD, Vector3d Bestvel_ECEF_SD, double dualheading);
    bool Initialized() { return initialized_; } // returns whether the INS has been initialized
    void Update(int num, Vector3d Meas_omega_ib_b, Vector3d Meas_f_ib_b, Vector3d Meas_r_eb_e, Vector3d Meas_v_eb_e, Vector3d Bestpos_ECEF_SD, Vector3d Bestvel_ECEF_SD, double dualheading);
  ...
  private:
    ...
    double aNoiseSigma_mps2 = 0.05; // 0.05
    double aMarkovSigma_mps2 = 0.01;
    double aMarkovTau_s = 100.0;

    double wNoiseSigma_rps2 = 0.00175;
    double wMarkovSigma_rps2 = 0.00025;
    ...
    double aBiasSigma_Init_mps2 = 0.9810; //sqrt(9.6170e-05);// Std dev of initial acceleration bias (m/s^2)
    ...
    // Methods
    void System_Update(double dualheading);
    void Measurement_Update(Vector3d pMeas_ECEF_m, Vector3d vMeas_ECEF_mps);
};
